 ...
 *
 * Registers:
 *   wp wake version  — theme name and version, read from WAKE_SLUG + WAKE_VERSION.
...
class Wake_CLI_Command {
	...
	public function version(): void {
		$theme = wp_get_theme();
		\WP_CLI::log( sprintf( '%s %s', (string) $theme->get( 'Name' ), WAKE_VERSION ) );
	}
	...
	public function info(): void {
		...
		$languages_dir = WAKE_DIR . '/languages';
		$pot_path      = $languages_dir . '/' . WAKE_SLUG . '.pot';
		$has_pot       = is_dir( $languages_dir ) && file_exists( $pot_path );

		\WP_CLI::log( 'Name:            ' . (string) $theme->get( 'Name' ) );
		\WP_CLI::log( 'Slug:            ' . WAKE_SLUG );
		\WP_CLI::log( 'Version:         ' . WAKE_VERSION );
		\WP_CLI::log( 'Active template: ' . $template );
		\WP_CLI::log( 'languages/.pot:  ' . ( $has_pot ? 'present' : 'missing — run: wp i18n make-pot . languages/' . WAKE_SLUG . '.pot' ) );
	}
...
\WP_CLI::add_command( 'wake', 'Wake_CLI_Command' );
