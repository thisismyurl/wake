 ...
 *
 *     add_filter( 'wake/github_updater_repo', static function () {
 *         return 'owner/repo';
 *     } );
 *
...
function github_updater_repo(): string {
	 ...
	 */
	$repo = (string) apply_filters( 'wake/github_updater_repo', '' );
	$repo = trim( $repo );
...
function github_updater_fetch_release( string $repo ): ?array {
	$cache_key = 'wake_gh_release_' . md5( SLUG . '|' . $repo );
...
function github_updater_flush_cache(): void {
	...
	delete_transient( 'wake_gh_release_' . md5( SLUG . '|' . $repo ) );
}
